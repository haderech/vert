...
export namespace Asset {
    ...
    export class Symbol implements ABISerializableObject {
        ...
        get code(): SymbolCode {
            return new SymbolCode(UInt64.from(this.value.value >> 8n))
        }
        ...
        convertUnits(units: Int64): number {
            return units.toNumber() / Math.pow(10, this.precision)
        }
    ...
    export type SymbolCodeType = SymbolCode | UInt64 | string | number | bigint
    export class SymbolCode implements ABISerializableObject {
        ...
        toString() {
            return charsToSymbolName([...this.value.byteArray.reverse()])
        }
...
function toSymbolPrecision(rawSymbol: UInt64) {
    return Number(rawSymbol.value & 255n)
}

function toSymbolName(rawSymbol: UInt64) {
    const chars = rawSymbol.byteArray.reverse().slice(0, -1)
    return charsToSymbolName([...chars])
}

function charsToSymbolName(chars: number[]) {
    return chars
        .map((char) => char ? String.fromCharCode(char) : '')
        .reverse()
        .join('')
}
